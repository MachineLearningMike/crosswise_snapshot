...
async function replayTransferEvents(decimals, transferEvents, initialSupply, owner) {
    ...
    for (let index = 0; index < transferEvents.length; index++) {
        ...
        console.log(sender, recipient, state, `${index}`.green);
        console.log(`${transferEvents[index].transactionHash}`.yellow)
...
function writeBalancesToExcel(balances, filename) {
    ...
    balances.forEach(function (row) {
        if (true) {  //row[1] != 0 ) {
            ...
            var contract = " ";
            for (let i = 0; i < Object.values(knownAccounts).length; i++) {
